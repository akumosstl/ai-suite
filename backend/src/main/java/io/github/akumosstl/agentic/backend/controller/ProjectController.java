...
public class ProjectController {
    ...
    @GetMapping("/all/pipelines")
    public List<Map<String, Object>> getAllProjectPipelines() {
        List<Project> projects = projectRepository.findAll();
        List<Map<String, Object>> result = new ArrayList<>();
        for (Project project : projects) {
            List<Pipeline> pipelines = pipelineRepository.findByProject_IdOrderByCreatedAtDesc(project.getId());
            for (Pipeline pipeline : pipelines) {
                Map<String, Object> entry = new HashMap<>();
                entry.put("projectId", project.getId());
                entry.put("projectName", project.getName());
                entry.put("pipelineId", pipeline.getId());
                entry.put("pipelineName", pipeline.getName());
                entry.put("pipelineStatus", pipeline.getStatus());
                result.add(entry);
            }
        }
        return result;
