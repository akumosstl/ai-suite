...
export interface ProjectToolsDialogData {
  projectId: number;
}

@Component({
  selector: 'app-project-tools-dialog',
  standalone: true,
  ...
  template: `
    ...
    <mat-dialog-content class="dialog-content">
      <mat-tab-group [(selectedIndex)]="selectedTab" class="custom-tabs">
        <mat-tab>
          <ng-template mat-tab-label><mat-icon>list</mat-icon><span>Project Tools</span></ng-template>
          <div class="tab-content">
            <div class="table-container">
              <div *ngIf="loading" class="loading-overlay"><mat-progress-spinner diameter="40" mode="indeterminate"></mat-progress-spinner><span>Loading tools...</span></div>
              <div *ngIf="!loading && projectItems.length === 0" class="empty-state"><mat-icon class="empty-icon">build</mat-icon><span class="empty-text">No tools assigned</span><span class="empty-hint">Click the "Add Tools" tab to add tools to this project</span></div>
              <table mat-table [dataSource]="projectItems" class="item-table" *ngIf="!loading && projectItems.length > 0">
                <ng-container matColumnDef="name"><th mat-header-cell *matHeaderCellDef>Tool Name</th><td mat-cell *matCellDef="let item"><div class="item-name"><mat-icon class="item-icon">build</mat-icon>{{ item.name }}</div></td></ng-container>
                ...
                <ng-container matColumnDef="actions"><th mat-header-cell *matHeaderCellDef></th><td mat-cell *matCellDef="let item"><button mat-icon-button (click)="removeItem(item, $event)" class="remove-btn" title="Remove tool"><mat-icon>remove_circle</mat-icon></button></td></ng-container>
        ...
        <mat-tab>
          <ng-template mat-tab-label><mat-icon>add_circle</mat-icon><span>Add Tools to Project</span></ng-template>
          <div class="tab-content">
            <div class="search-section">
              <mat-form-field class="search-field" appearance="outline"><mat-label>Search tools...</mat-label><input matInput [(ngModel)]="searchTerm" (keyup.enter)="search()" placeholder="Type to search..."><mat-icon matPrefix>search</mat-icon></mat-form-field>
            ...
            <div class="table-container">
              <div *ngIf="loadingAvailable" class="loading-overlay"><mat-progress-spinner diameter="40" mode="indeterminate"></mat-progress-spinner><span>Loading tools...</span></div>
              <div *ngIf="!loadingAvailable && availableItems.length === 0" class="empty-state"><mat-icon class="empty-icon">build</mat-icon><span class="empty-text">No tools found</span><span class="empty-hint">Create a new tool or adjust your search</span></div>
    ...
    <mat-dialog-actions class="dialog-actions">
      ...
      <button mat-raised-button color="primary" (click)="onAdd()" [disabled]="selection.isEmpty()" class="add-btn" *ngIf="selectedTab === 1"><mat-icon>add</mat-icon>Add {{ selection.selected.length > 0 ? selection.selected.length + ' ' : '' }}Tool{{ selection.selected.length !== 1 ? 's' : '' }}</button>
    </mat-dialog-actions>
  `,
  styles: [`
    ...
    .header-icon { font-size: 28px; width: 28px; height: 28px; color: #a1887f; }
    ...
    ::ng-deep .custom-tabs .mat-mdc-tab.mdc-tab--active { color: #a1887f !important; }
    ::ng-deep .custom-tabs .mat-mdc-tab-indicator .mdc-tab-indicator__content--underline { border-color: #a1887f !important; }
    ...
    .item-icon { font-size: 20px; width: 20px; height: 20px; color: #a1887f; }
    .category-badge { display: inline-flex; align-items: center; padding: 4px 12px; border-radius: 20px; font-size: 0.8rem; font-weight: 500; text-transform: capitalize; background: rgba(161, 136, 127, 0.2); color: #a1887f; }
    ...
    ::ng-deep .mat-mdc-progress-spinner circle { stroke: #a1887f !important; }
    ::ng-deep .mat-mdc-checkbox .mdc-checkbox__background { border-color: #888 !important; }
    ::ng-deep .mat-mdc-checkbox.mat-mdc-checkbox-checked .mdc-checkbox__background { background-color: #a1887f !important; border-color: #a1887f !important; }
  `],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class ProjectToolsDialogComponent {
  selectedTab = 0;
  searchTerm = '';
  projectItems: Tool[] = [];
  availableItems: Tool[] = [];
  ...
  selection: { isSelected: (row: Tool) => boolean; hasValue: () => boolean; selected: Tool[]; toggle: (row: Tool) => void; clear: () => void; isEmpty: () => boolean } = { isSelected: () => false, hasValue: () => false, selected: [], toggle: () => {}, clear: () => {}, isEmpty: () => true };
  constructor(public dialogRef: MatDialogRef<ProjectToolsDialogComponent>, private apiService: ApiService, private cdr: ChangeDetectorRef, @Inject(MAT_DIALOG_DATA) public data: ProjectToolsDialogData) { this.loadProjectItems(); this.loadAvailableItems(); this.initSelection(); }
  private initSelection(): void { const selectedItems: Tool[] = []; this.selection = { isSelected: (row: Tool) => selectedItems.some(s => s.id === row.id), hasValue: () => selectedItems.length > 0, get selected() { return selectedItems; }, toggle: (row: Tool) => { const index = selectedItems.findIndex(s => s.id === row.id); if (index >= 0) { selectedItems.splice(index, 1); } else { selectedItems.push(row); } this.cdr.detectChanges(); }, clear: () => { selectedItems.length = 0; this.cdr.detectChanges(); }, isEmpty: () => selectedItems.length === 0 }; }
  loadProjectItems(): void { this.loading = true; this.apiService.getProjectTools(this.data.projectId).subscribe({ next: (items) => { this.projectItems = items || []; this.loading = false; this.cdr.detectChanges(); }, error: (err) => { console.error('Error loading project tools:', err); this.loading = false; this.cdr.detectChanges(); } }); }
  loadAvailableItems(): void { this.loadingAvailable = true; if (this.searchTerm.trim()) { this.apiService.searchTools(this.searchTerm, '', this.currentPage, this.pageSize).subscribe({ next: (response) => { this.handleResponse(response); }, error: (err) => { console.error('Error searching tools:', err); this.loadingAvailable = false; } }); } else { this.apiService.getTools(this.currentPage, this.pageSize).subscribe({ next: (response) => { this.handleResponse(response); }, error: (err) => { console.error('Error loading tools:', err); this.loadingAvailable = false; } }); } }
  handleResponse(response: any): void { if (response && Array.isArray(response.tools)) { this.availableItems = response.tools; } else if (Array.isArray(response)) { this.availableItems = response; } else { this.availableItems = []; } this.totalElements = response?.totalElements ?? this.availableItems.length; this.loadingAvailable = false; this.cdr.detectChanges(); }
  ...
  toggleRow(row: Tool): void { this.selection.toggle(row); }
  ...
  removeItem(item: Tool, event: Event): void { event.stopPropagation(); if (item.id) { this.apiService.removeToolFromProject(this.data.projectId, item.id).subscribe({ next: () => { this.loadProjectItems(); }, error: (err) => { console.error('Error removing tool:', err); } }); } }
  onAdd(): void { if (this.selection.selected.length > 0 && this.data.projectId) { const itemIds = this.selection.selected.filter(s => s.id).map(s => s.id as number); this.apiService.addToolsToProject(this.data.projectId, itemIds).subscribe({ next: () => { this.dialogRef.close(this.selection.selected); }, error: (err) => { console.error('Error adding tools to project:', err); } }); } }
