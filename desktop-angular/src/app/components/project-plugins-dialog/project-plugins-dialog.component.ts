...
export interface ProjectPluginsDialogData {
  projectId: number;
}

@Component({
  selector: 'app-project-plugins-dialog',
  standalone: true,
  ...
  template: `
    ...
    <mat-dialog-content class="dialog-content">
      <mat-tab-group [(selectedIndex)]="selectedTab" class="custom-tabs">
        <mat-tab>
          <ng-template mat-tab-label><mat-icon>list</mat-icon><span>Project Plugins</span></ng-template>
          <div class="tab-content">
            <div class="table-container">
              <div *ngIf="loading" class="loading-overlay"><mat-progress-spinner diameter="40" mode="indeterminate"></mat-progress-spinner><span>Loading plugins...</span></div>
              <div *ngIf="!loading && projectItems.length === 0" class="empty-state"><mat-icon class="empty-icon">extension</mat-icon><span class="empty-text">No plugins assigned</span><span class="empty-hint">Click the "Add Plugins" tab to add plugins to this project</span></div>
              <table mat-table [dataSource]="projectItems" class="item-table" *ngIf="!loading && projectItems.length > 0">
                <ng-container matColumnDef="name"><th mat-header-cell *matHeaderCellDef>Plugin Name</th><td mat-cell *matCellDef="let item"><div class="item-name"><mat-icon class="item-icon">extension</mat-icon>{{ item.name }}</div></td></ng-container>
                ...
                <ng-container matColumnDef="actions"><th mat-header-cell *matHeaderCellDef></th><td mat-cell *matCellDef="let item"><button mat-icon-button (click)="removeItem(item, $event)" class="remove-btn" title="Remove plugin"><mat-icon>remove_circle</mat-icon></button></td></ng-container>
        ...
        <mat-tab>
          <ng-template mat-tab-label><mat-icon>add_circle</mat-icon><span>Add Plugins to Project</span></ng-template>
          <div class="tab-content">
            <div class="search-section">
              <mat-form-field class="search-field" appearance="outline"><mat-label>Search plugins...</mat-label><input matInput [(ngModel)]="searchTerm" (keyup.enter)="search()" placeholder="Type to search..."><mat-icon matPrefix>search</mat-icon></mat-form-field>
            ...
            <div class="table-container">
              <div *ngIf="loadingAvailable" class="loading-overlay"><mat-progress-spinner diameter="40" mode="indeterminate"></mat-progress-spinner><span>Loading plugins...</span></div>
              <div *ngIf="!loadingAvailable && availableItems.length === 0" class="empty-state"><mat-icon class="empty-icon">extension</mat-icon><span class="empty-text">No plugins found</span><span class="empty-hint">Create a new plugin or adjust your search</span></div>
    ...
    <mat-dialog-actions class="dialog-actions">
      ...
      <button mat-raised-button color="primary" (click)="onAdd()" [disabled]="selection.isEmpty()" class="add-btn" *ngIf="selectedTab === 1"><mat-icon>add</mat-icon>Add {{ selection.selected.length > 0 ? selection.selected.length + ' ' : '' }}Plugin{{ selection.selected.length !== 1 ? 's' : '' }}</button>
    </mat-dialog-actions>
  `,
  styles: [`
    ...
    .header-icon { font-size: 28px; width: 28px; height: 28px; color: #f06292; }
    ...
    ::ng-deep .custom-tabs .mat-mdc-tab.mdc-tab--active { color: #f06292 !important; }
    ::ng-deep .custom-tabs .mat-mdc-tab-indicator .mdc-tab-indicator__content--underline { border-color: #f06292 !important; }
    ...
    .item-icon { font-size: 20px; width: 20px; height: 20px; color: #f06292; }
    .category-badge { display: inline-flex; align-items: center; padding: 4px 12px; border-radius: 20px; font-size: 0.8rem; font-weight: 500; text-transform: capitalize; background: rgba(240, 98, 146, 0.2); color: #f06292; }
    ...
    ::ng-deep .mat-mdc-progress-spinner circle { stroke: #f06292 !important; }
    ::ng-deep .mat-mdc-checkbox .mdc-checkbox__background { border-color: #888 !important; }
    ::ng-deep .mat-mdc-checkbox.mat-mdc-checkbox-checked .mdc-checkbox__background { background-color: #f06292 !important; border-color: #f06292 !important; }
  `],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class ProjectPluginsDialogComponent {
  selectedTab = 0;
  searchTerm = '';
  projectItems: Plugin[] = [];
  availableItems: Plugin[] = [];
  loading = false;
  ...
  selection: { isSelected: (row: Plugin) => boolean; hasValue: () => boolean; selected: Plugin[]; toggle: (row: Plugin) => void; clear: () => void; isEmpty: () => boolean } = { isSelected: () => false, hasValue: () => false, selected: [], toggle: () => {}, clear: () => {}, isEmpty: () => true };
  constructor(public dialogRef: MatDialogRef<ProjectPluginsDialogComponent>, private apiService: ApiService, private cdr: ChangeDetectorRef, @Inject(MAT_DIALOG_DATA) public data: ProjectPluginsDialogData) { this.loadProjectItems(); this.loadAvailableItems(); this.initSelection(); }
  private initSelection(): void { const selectedItems: Plugin[] = []; this.selection = { isSelected: (row: Plugin) => selectedItems.some(s => s.id === row.id), hasValue: () => selectedItems.length > 0, get selected() { return selectedItems; }, toggle: (row: Plugin) => { const index = selectedItems.findIndex(s => s.id === row.id); if (index >= 0) { selectedItems.splice(index, 1); } else { selectedItems.push(row); } this.cdr.detectChanges(); }, clear: () => { selectedItems.length = 0; this.cdr.detectChanges(); }, isEmpty: () => selectedItems.length === 0 }; }
  loadProjectItems(): void { this.loading = true; this.apiService.getProjectPlugins(this.data.projectId).subscribe({ next: (items) => { this.projectItems = items || []; this.loading = false; this.cdr.detectChanges(); }, error: (err) => { console.error('Error loading project plugins:', err); this.loading = false; this.cdr.detectChanges(); } }); }
  loadAvailableItems(): void { this.loadingAvailable = true; if (this.searchTerm.trim()) { this.apiService.searchPlugins(this.searchTerm, '', this.currentPage, this.pageSize).subscribe({ next: (response) => { this.handleResponse(response); }, error: (err) => { console.error('Error searching plugins:', err); this.loadingAvailable = false; } }); } else { this.apiService.getPlugins(this.currentPage, this.pageSize).subscribe({ next: (response) => { this.handleResponse(response); }, error: (err) => { console.error('Error loading plugins:', err); this.loadingAvailable = false; } }); } }
  handleResponse(response: any): void { if (response && Array.isArray(response.plugins)) { this.availableItems = response.plugins; } else if (Array.isArray(response)) { this.availableItems = response; } else { this.availableItems = []; } this.totalElements = response?.totalElements ?? this.availableItems.length; this.loadingAvailable = false; this.cdr.detectChanges(); }
  ...
  toggleRow(row: Plugin): void { this.selection.toggle(row); }
  ...
  removeItem(item: Plugin, event: Event): void { event.stopPropagation(); if (item.id) { this.apiService.removePluginFromProject(this.data.projectId, item.id).subscribe({ next: () => { this.loadProjectItems(); }, error: (err) => { console.error('Error removing plugin:', err); } }); } }
  onAdd(): void { if (this.selection.selected.length > 0 && this.data.projectId) { const itemIds = this.selection.selected.filter(s => s.id).map(s => s.id as number); this.apiService.addPluginsToProject(this.data.projectId, itemIds).subscribe({ next: () => { this.dialogRef.close(this.selection.selected); }, error: (err) => { console.error('Error adding plugins to project:', err); } }); } }
