  ...
  template: `
    ...
    <mat-dialog-content class="dialog-content">
      <mat-tab-group [(selectedIndex)]="selectedTab" class="custom-tabs">
        <mat-tab>
          <ng-template mat-tab-label><mat-icon>list</mat-icon><span>Project Scripts</span></ng-template>
          <div class="tab-content">
            <div class="table-container">
              <div *ngIf="loading" class="loading-overlay"><mat-progress-spinner diameter="40" mode="indeterminate"></mat-progress-spinner><span>Loading scripts...</span></div>
              <div *ngIf="!loading && projectItems.length === 0" class="empty-state"><mat-icon class="empty-icon">code</mat-icon><span class="empty-text">No scripts assigned</span><span class="empty-hint">Click the "Add Scripts" tab to add scripts to this project</span></div>
              <table mat-table [dataSource]="projectItems" class="item-table" *ngIf="!loading && projectItems.length > 0">
                <ng-container matColumnDef="name"><th mat-header-cell *matHeaderCellDef>Script Name</th><td mat-cell *matCellDef="let item"><div class="item-name"><mat-icon class="item-icon">code</mat-icon>{{ item.name }}</div></td></ng-container>
                ...
                <ng-container matColumnDef="actions"><th mat-header-cell *matHeaderCellDef></th><td mat-cell *matCellDef="let item"><button mat-icon-button (click)="removeItem(item, $event)" class="remove-btn" title="Remove script"><mat-icon>remove_circle</mat-icon></button></td></ng-container>
        ...
        <mat-tab>
          <ng-template mat-tab-label><mat-icon>add_circle</mat-icon><span>Add Scripts to Project</span></ng-template>
          <div class="tab-content">
            <div class="search-section">
              <mat-form-field class="search-field" appearance="outline"><mat-label>Search scripts...</mat-label><input matInput [(ngModel)]="searchTerm" (keyup.enter)="search()" placeholder="Type to search..."><mat-icon matPrefix>search</mat-icon></mat-form-field>
            ...
            <div class="table-container">
              <div *ngIf="loadingAvailable" class="loading-overlay"><mat-progress-spinner diameter="40" mode="indeterminate"></mat-progress-spinner><span>Loading scripts...</span></div>
              <div *ngIf="!loadingAvailable && availableItems.length === 0" class="empty-state"><mat-icon class="empty-icon">code</mat-icon><span class="empty-text">No scripts found</span><span class="empty-hint">Create a new script or adjust your search</span></div>
    ...
    <mat-dialog-actions class="dialog-actions">
      ...
      <button mat-raised-button color="primary" (click)="onAdd()" [disabled]="selection.isEmpty()" class="add-btn" *ngIf="selectedTab === 1"><mat-icon>add</mat-icon>Add {{ selection.selected.length > 0 ? selection.selected.length + ' ' : '' }}Script{{ selection.selected.length !== 1 ? 's' : '' }}</button>
    </mat-dialog-actions>
  `,
  styles: [`
    ...
    .header-icon { font-size: 28px; width: 28px; height: 28px; color: #ba68c8; }
    ...
    ::ng-deep .custom-tabs .mat-mdc-tab.mdc-tab--active { color: #ba68c8 !important; }
    ::ng-deep .custom-tabs .mat-mdc-tab-indicator .mdc-tab-indicator__content--underline { border-color: #ba68c8 !important; }
    ...
    .item-icon { font-size: 20px; width: 20px; height: 20px; color: #ba68c8; }
    .category-badge { display: inline-flex; align-items: center; padding: 4px 12px; border-radius: 20px; font-size: 0.8rem; font-weight: 500; text-transform: capitalize; background: rgba(186, 104, 200, 0.2); color: #ba68c8; }
    ...
    ::ng-deep .mat-mdc-progress-spinner circle { stroke: #ba68c8 !important; }
    ::ng-deep .mat-mdc-checkbox .mdc-checkbox__background { border-color: #888 !important; }
    ::ng-deep .mat-mdc-checkbox.mat-mdc-checkbox-checked .mdc-checkbox__background { background-color: #ba68c8 !important; border-color: #ba68c8 !important; }
  `],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class ProjectScriptsDialogComponent {
  selectedTab = 0;
  searchTerm = '';
  projectItems: Script[] = [];
  availableItems: Script[] = [];
  loading = false;
  ...
  handleResponse(response: any): void { if (response && Array.isArray(response.scripts)) { this.availableItems = response.scripts; } else if (Array.isArray(response)) { this.availableItems = response; } else { this.availableItems = []; } this.totalElements = response?.totalElements ?? this.availableItems.length; this.loadingAvailable = false; this.cdr.detectChanges(); }
  ...
  toggleRow(row: Script): void { this.selection.toggle(row); }
