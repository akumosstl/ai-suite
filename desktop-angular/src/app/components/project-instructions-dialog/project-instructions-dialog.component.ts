  ...
  template: `
    ...
    <mat-dialog-content class="dialog-content">
      <mat-tab-group [(selectedIndex)]="selectedTab" class="custom-tabs">
        <mat-tab>
          <ng-template mat-tab-label><mat-icon>list</mat-icon><span>Project Instructions</span></ng-template>
          <div class="tab-content">
            <div class="table-container">
              <div *ngIf="loading" class="loading-overlay"><mat-progress-spinner diameter="40" mode="indeterminate"></mat-progress-spinner><span>Loading instructions...</span></div>
              <div *ngIf="!loading && projectItems.length === 0" class="empty-state"><mat-icon class="empty-icon">menu_book</mat-icon><span class="empty-text">No instructions assigned</span><span class="empty-hint">Click the "Add Instructions" tab to add instructions to this project</span></div>
              <table mat-table [dataSource]="projectItems" class="item-table" *ngIf="!loading && projectItems.length > 0">
                <ng-container matColumnDef="name"><th mat-header-cell *matHeaderCellDef>Instruction Name</th><td mat-cell *matCellDef="let item"><div class="item-name"><mat-icon class="item-icon">menu_book</mat-icon>{{ item.name }}</div></td></ng-container>
                ...
                <ng-container matColumnDef="actions"><th mat-header-cell *matHeaderCellDef></th><td mat-cell *matCellDef="let item"><button mat-icon-button (click)="removeItem(item, $event)" class="remove-btn" title="Remove instruction"><mat-icon>remove_circle</mat-icon></button></td></ng-container>
        ...
        <mat-tab>
          <ng-template mat-tab-label><mat-icon>add_circle</mat-icon><span>Add Instructions to Project</span></ng-template>
          <div class="tab-content">
            <div class="search-section">
              <mat-form-field class="search-field" appearance="outline"><mat-label>Search instructions...</mat-label><input matInput [(ngModel)]="searchTerm" (keyup.enter)="search()" placeholder="Type to search..."><mat-icon matPrefix>search</mat-icon></mat-form-field>
            ...
            <div class="table-container">
              <div *ngIf="loadingAvailable" class="loading-overlay"><mat-progress-spinner diameter="40" mode="indeterminate"></mat-progress-spinner><span>Loading instructions...</span></div>
              <div *ngIf="!loadingAvailable && availableItems.length === 0" class="empty-state"><mat-icon class="empty-icon">menu_book</mat-icon><span class="empty-text">No instructions found</span><span class="empty-hint">Create a new instruction or adjust your search</span></div>
    ...
    <mat-dialog-actions class="dialog-actions">
      ...
      <button mat-raised-button color="primary" (click)="onAdd()" [disabled]="selection.isEmpty()" class="add-btn" *ngIf="selectedTab === 1"><mat-icon>add</mat-icon>Add {{ selection.selected.length > 0 ? selection.selected.length + ' ' : '' }}Instruction{{ selection.selected.length !== 1 ? 's' : '' }}</button>
    </mat-dialog-actions>
  `,
  styles: [`
    ...
    .header-icon { font-size: 28px; width: 28px; height: 28px; color: #4dd0e1; }
    ...
    ::ng-deep .custom-tabs .mat-mdc-tab.mdc-tab--active { color: #4dd0e1 !important; }
    ::ng-deep .custom-tabs .mat-mdc-tab-indicator .mdc-tab-indicator__content--underline { border-color: #4dd0e1 !important; }
    ...
    .item-icon { font-size: 20px; width: 20px; height: 20px; color: #4dd0e1; }
    .category-badge { display: inline-flex; align-items: center; padding: 4px 12px; border-radius: 20px; font-size: 0.8rem; font-weight: 500; text-transform: capitalize; background: rgba(77, 208, 225, 0.2); color: #4dd0e1; }
    ...
    ::ng-deep .mat-mdc-progress-spinner circle { stroke: #4dd0e1 !important; }
    ::ng-deep .mat-mdc-checkbox .mdc-checkbox__background { border-color: #888 !important; }
    ::ng-deep .mat-mdc-checkbox.mat-mdc-checkbox-checked .mdc-checkbox__background { background-color: #4dd0e1 !important; border-color: #4dd0e1 !important; }
  `],
...
export class ProjectInstructionsDialogComponent {
  selectedTab = 0;
  searchTerm = '';
  projectItems: Instruction[] = [];
  availableItems: Instruction[] = [];
  loading = false;
  ...
  selection: { isSelected: (row: Instruction) => boolean; hasValue: () => boolean; selected: Instruction[]; toggle: (row: Instruction) => void; clear: () => void; isEmpty: () => boolean } = { isSelected: () => false, hasValue: () => false, selected: [], toggle: () => {}, clear: () => {}, isEmpty: () => true };
  constructor(public dialogRef: MatDialogRef<ProjectInstructionsDialogComponent>, private apiService: ApiService, private cdr: ChangeDetectorRef, private dialog: MatDialog, @Inject(MAT_DIALOG_DATA) public data: ProjectInstructionsDialogData) { this.loadProjectItems(); this.loadAvailableItems(); this.initSelection(); }
  private initSelection(): void { const selectedItems: Instruction[] = []; this.selection = { isSelected: (row: Instruction) => selectedItems.some(s => s.id === row.id), hasValue: () => selectedItems.length > 0, get selected() { return selectedItems; }, toggle: (row: Instruction) => { const index = selectedItems.findIndex(s => s.id === row.id); if (index >= 0) { selectedItems.splice(index, 1); } else { selectedItems.push(row); } this.cdr.detectChanges(); }, clear: () => { selectedItems.length = 0; this.cdr.detectChanges(); }, isEmpty: () => selectedItems.length === 0 }; }
  loadProjectItems(): void { this.loading = true; this.apiService.getProjectInstructions(this.data.projectId).subscribe({ next: (items) => { this.projectItems = items || []; this.loading = false; this.cdr.detectChanges(); }, error: (err) => { console.error('Error loading project instructions:', err); this.loading = false; this.cdr.detectChanges(); } }); }
  loadAvailableItems(): void { this.loadingAvailable = true; if (this.searchTerm.trim()) { this.apiService.searchInstructions(this.searchTerm, '', this.currentPage, this.pageSize).subscribe({ next: (response) => { this.handleResponse(response); }, error: (err) => { console.error('Error searching instructions:', err); this.loadingAvailable = false; } }); } else { this.apiService.getInstructions(this.currentPage, this.pageSize).subscribe({ next: (response) => { this.handleResponse(response); }, error: (err) => { console.error('Error loading instructions:', err); this.loadingAvailable = false; } }); } }
  handleResponse(response: any): void { if (response && Array.isArray(response.instructions)) { this.availableItems = response.instructions; } else if (Array.isArray(response)) { this.availableItems = response; } else { this.availableItems = []; } this.totalElements = response?.totalElements ?? this.availableItems.length; this.loadingAvailable = false; this.cdr.detectChanges(); }
  ...
  toggleRow(row: Instruction): void { this.selection.toggle(row); }
  ...
  removeItem(item: Instruction, event: Event): void { event.stopPropagation(); if (item.id) { this.apiService.removeInstructionFromProject(this.data.projectId, item.id).subscribe({ next: () => { this.loadProjectItems(); }, error: (err) => { console.error('Error removing instruction:', err); } }); } }
  onAdd(): void { if (this.selection.selected.length > 0 && this.data.projectId) { const itemIds = this.selection.selected.filter(s => s.id).map(s => s.id as number); this.apiService.addInstructionsToProject(this.data.projectId, itemIds).subscribe({ next: () => { this.dialogRef.close(this.selection.selected); }, error: (err) => { if (err.status === 409 && err.error?.error === 'FILE_ALREADY_EXISTS') { this.showOverwriteConfirmation(err.error.message, () => { this.apiService.addInstructionsToProject(this.data.projectId, itemIds, true).subscribe({ next: () => { this.dialogRef.close(this.selection.selected); }, error: (retryErr) => { console.error('Error adding instructions to project:', retryErr); } }); }); } else { console.error('Error adding instructions to project:', err); } } }); } } showOverwriteConfirmation(fileName: string, onConfirm: () => void): void { const dialogRef = this.dialog.open(ConfirmDialogComponent, { width: '400px', data: { title: 'File Already Exists', message: `${fileName} already exists. Do you want to overwrite it?` }, panelClass: 'custom-dialog-panel' }); dialogRef.afterClosed().subscribe((result) => { if (result) { onConfirm(); } }); }
