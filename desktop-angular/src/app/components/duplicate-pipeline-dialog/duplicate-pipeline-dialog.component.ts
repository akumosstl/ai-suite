...
export interface DuplicatePipelineData {
  name: string;
}

@Component({
  selector: 'app-duplicate-pipeline-dialog',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  template: `
    <div class="dialog-header">
      ...
      <h2 class="dialog-title">Duplicate Pipeline</h2>
    </div>
    ...
    <mat-dialog-actions class="dialog-actions">
      ...
      <button mat-raised-button color="primary" (click)="onCreate()" [disabled]="!pipelineName" class="create-btn">
...
export class DuplicatePipelineDialogComponent implements OnInit {
  pipelineName: string = '';

  constructor(
    public dialogRef: MatDialogRef<DuplicatePipelineDialogComponent>,
    @Inject(MAT_DIALOG_DATA) public data: DuplicatePipelineData
  ) {}
  ...
  onCreate(): void {
    if (this.pipelineName.trim()) {
      this.dialogRef.close(this.pipelineName.trim());
    }
  }
